        ...
        engine.generate(context, request, object : GemmaEngine.Callback {
            ...
            override fun onComplete(response: String, emotion: String?, timing: GemmaTiming) {
                call.resolve(JSObject().apply {
                    ...
                    put("temperature", timing.temperature)
                    put("topK", timing.topK)
                    put("topP", timing.topP)
                    put("randomSeed", timing.randomSeed)
                    put("samplingEnabled", timing.samplingEnabled)
                    put("historyIncluded", timing.historyIncluded)
                    put("systemIncluded", timing.systemIncluded)
                    put("processedResponse", timing.processedResponse)
                    put("processedEmotion", timing.processedEmotion)
